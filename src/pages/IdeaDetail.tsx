...
export default function IdeaDetail() {
  ...
  const handleInvest = () => {
    toast({ title: t.common.success, description: "Interest expressed! The founder will be notified." });
  };
  ...
  return (
    <div className="container mx-auto px-4 py-10 max-w-5xl">
      ...
      {/* Header */}
      <div className="glass rounded-2xl p-6 md:p-8 shadow-glass mb-6">
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
          <div className="flex-1">
            ...
            <div className="flex items-center gap-3 text-sm text-muted-foreground">
              ...
              {idea.location && (
                <>
                  <span>•</span>
                  <span className="flex items-center gap-1"><MapPin className="h-3 w-3" />{idea.location}</span>
                </>
              )}
              <span>•</span>
              <span>{t.ideaDetail.postedOn} {new Date(idea.created_at).toLocaleDateString()}</span>
            </div>
          </div>
          <div className="flex gap-2 shrink-0">
            ...
            <Button size="sm" className="gradient-primary border-0 text-primary-foreground" onClick={handleInvest}>
              <DollarSign className="h-4 w-4 me-1" />
              {t.ideaDetail.investBtn}
            </Button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {scores.map((s, i) => (
          <div key={i} className="glass rounded-xl p-4 shadow-glass text-center">
      ...
      <Tabs defaultValue="overview" className="glass rounded-2xl shadow-glass overflow-hidden">
        <TabsList className="w-full justify-start bg-muted/50 rounded-none border-b border-border/50 px-4">
          ...
          <TabsTrigger value="details">{t.ideaDetail.financialPotential}</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="evaluation" className="p-6">
          {idea.ai_evaluation ? (
            <div
              ...
              dangerouslySetInnerHTML={{ __html: markdownToHtml(idea.ai_evaluation) }}
            />
          ) : (
